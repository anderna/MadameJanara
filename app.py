...
# --- 2. LAYOUT MÍSTICO E RESPONSIVO ---
st.set_page_config(page_title="Madame Janara v4.4", layout="centered")
...
st.markdown("""
    ...
    .stButton>button:hover { background: #fff !important; box-shadow: 0 0 20px #d4af37; transform: scale(1.02); }
...
# --- 3. GESTÃO DE ESTADO (LGPD & PERSISTÊNCIA) ---
...
# --- 4. MOTOR DE IA (RESILIÊNCIA E CONTEÚDO DENSO) ---
...
def gerar_profecia(imagem, d):
    ...
    prompt = f"""
    Você é Madame Janara, uma cigana sábia de linhagem antiga. Sua voz é profunda, detalhada e mística.
    DADOS: {d['nome']}, Nasc: {d['data_nasc']} às {d['horario']} em {d['cidade']}, Signo: {d['signo']}.
    PARCEIRO: {d['p_nome']}, Signo: {d['p_signo']}.

    Gere 4 partes separadas por '---'. NÃO escreva 'Parte X' ou 'Bloco'.
    
    1 (TEASER): Um resumo místico e impactante de no máximo 2 frases.
    ---
    2 (PADRÃO): Leitura das mãos cruzada com o signo solar. Seja detalhista sobre as linhas (Vida, Cabeça, Coração). Mínimo de 150 palavras.
    ---
    3 (VIP): Mapa Astral e Destino. Analise o impacto do Ascendente e da Lua no sucesso financeiro e pessoal. Seja prolixa, use metáforas ricas e fale sobre os próximos 12 meses em detalhes. Mínimo de 250 palavras.
    ---
    4 (SOULMATE): Sinastria mística entre {d['nome']} e {d['p_nome']}. Explore a fundo a compatibilidade de {d['signo']} com {d['p_signo']}. Revele um segredo de vidas passadas e dê 3 conselhos práticos e detalhados para a harmonia do casal. Mínimo de 200 palavras.

    TOM: Use português do Brasil. Seja carismática, misteriosa e extremamente detalhista nos níveis 3 e 4.
    """
    
    imagem.thumbnail((800, 800))
    # Configuração para garantir profundidade e criatividade
    config = genai.types.GenerationConfig(max_output_tokens=2048, temperature=0.8)
...
with st.container():
    ...
    c1, c2 = st.columns(2)
    with c1:
        nome = st.text_input("Seu nome:", placeholder="Como deseja ser chamado(a)?", key=f"nome_{fid}")
        data_nasc = st.date_input("Data de Nascimento:", value=None, min_value=date(1900, 1, 1), format="DD/MM/YYYY", key=f"data_{fid}")
        cidade = st.text_input("Cidade de Origem:", placeholder="Onde você nasceu?", key=f"cid_{fid}")
    with c2:
        signo = st.selectbox("Seu Signo Solar:", ["Escolha...", "Áries", "Touro", "Gêmeos", "Câncer", "Leão", "Virgem", "Libra", "Escorpião", "Sagitário", "Capricórnio", "Aquário", "Peixes"], key=f"sig_{fid}")
        horario = st.time_input("Hora do Nascimento:", value=None, key=f"hora_{fid}")
    
    st.markdown("---")
    st.subheader("❤️ Conexão de Almas (Opcional)")
    p_nome = st.text_input("Nome do Parceiro(a):", key=f"pnome_{fid}", placeholder="Deixe vazio para consulta individual")
    p_signo = st.selectbox("Signo do Parceiro(a):", ["Não informado", "Áries", "Touro", "Gêmeos", "Câncer", "Leão", "Virgem", "Libra", "Escorpião", "Sagitário", "Capricórnio", "Aquário", "Peixes"], key=f"psig_{fid}")

    uploaded_file = st.file_uploader("Sua palma esquerda (clara e visível):", type=["jpg", "jpeg", "png"], key=f"file_{fid}")
            ...
            if uploaded_file and nome and data_nasc and signo != "Escolha...":
                with st.spinner("Madame Janara acessa as correntes do tempo..."):
                    try:
                        ...
                        st.error(f"As névoas estão densas. Erro: {e}")
            else:
                st.warning("Preencha os dados obrigatórios e envie a foto da mão.")
    
    with col_btn2:
        st.button("🗑️ Limpar", on_click=limpar_tudo)
    st.markdown("</div>", unsafe_allow_html=True)

# --- 6. EXIBIÇÃO DE RESULTADOS (ESCADA DE VALOR) ---
...
if res["feita"]:
    ...
    if res["soulmate"] and p_nome:
        st.markdown(f"<div class='soulmate-section'><h3>🌹 Destino a Dois: {nome} & {p_nome}</h3>", unsafe_allow_html=True)
        email = st.text_input("E-mail para o guia de casal completo:", key=f"mail_{fid}")
        if st.button("💖 Desbloquear Almas"):
            if email:
                ...
                st.warning("Por favor, insira seu e-mail para desbloquear a sinastria.")
        st.markdown("</div>", unsafe_allow_html=True)