...
st.markdown("""
    ...
    .stButton>button { background: #d4af37 !important; color: black !important; font-weight: 900; border-radius: 50px; width: 100%; }
...
with st.container():
    ...
    c1, c2 = st.columns(2)
    with c1:
        nome = st.text_input("Seu nome:", placeholder="Nome completo", key=f"nome_{fid}")
        data_nasc = st.date_input("Nascimento:", value=None, min_value=date(1900, 1, 1), format="DD/MM/YYYY", key=f"data_{fid}")
        cidade = st.text_input("Cidade de Origem:", placeholder="Onde você nasceu?", key=f"cid_{fid}")
    with c2:
        signo = st.selectbox("Seu Signo:", ["Escolha...", "Áries", "Touro", "Gêmeos", "Câncer", "Leão", "Virgem", "Libra", "Escorpião", "Sagitário", "Capricórnio", "Aquário", "Peixes"], key=f"sig_{fid}")
        horario = st.time_input("Hora do Nascimento:", value=None, key=f"hora_{fid}")
    
    st.markdown("---")
    st.subheader("❤️ Conexão de Almas (Opcional)")
    p_nome = st.text_input("Nome do Parceiro(a):", key=f"pnome_{fid}")
    ...
    uploaded_file = st.file_uploader("Sua palma esquerda...", type=["jpg", "jpeg", "png"], key=f"file_{fid}")
...
if res["feita"]:
    ...
    with st.expander("🔓 Leitura das Mãos"):
        st.write(res["padrao"])
